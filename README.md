...
The SQLite schema includes columns reserved for Agent 2 (`policy_tags`, `document_ids`) that Cairn does not use but that the next agent will populate. These are architectural placeholders, not retrofits.
...
├── tests/
│   └── test_agent.py     # Brief validation across five test areas
├── app.py                # Streamlit wrapper (Phase 2)
├── load_data.py          # One-time migration: dementia CSV → SQLite
├── requirements.txt
├── .env.example
└── README.md
```
...
```bash
git clone https://github.com/clairvyn/cairn.git
cd cairn
python -m venv .venv
